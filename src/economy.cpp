 ...
 */
void ReserveConsist(Station *st, Vehicle *u, StationID next_station)
{
	if (_settings_game.order.improved_load && (u->current_order.GetLoadType() & OLFB_FULL_LOAD)) {
		...
		for (Vehicle *v = u; v != NULL; v = v->Next()) {
			...
			if (cap > 0) {
				...
				st->goods[v->cargo_type].cargo.MoveTo(&v->cargo, cap, next_station, true);
			}
		}
	}
}
...
static void LoadUnloadVehicle(Vehicle *front)
{
	...
	ReserveConsist(st, front, next_station);
	...
	for (Vehicle *v = front; v != NULL; v = v->Next()) {
		...
		if (front->current_order.IsRefit() && artic_part == 1 && IsArticulatedVehicleEmpty(v) &&
			...
			do {
				consist_capleft[w->cargo_type] += w->cargo_cap;
				if (do_reserve) {
					st->goods[w->cargo_type].cargo.MoveTo(&w->cargo, w->cargo_cap, next_station, true);
				}
