...
static void ReserveConsist(Station *st, Vehicle *u, CargoArray *consist_capleft, StationID next_station)
{
	...
	while (next_cargo != NULL) {
		...
		for (; v != NULL; v = v->Next()) {
			...
			if (cap > 0) {
				cap -= st->goods[v->cargo_type].cargo.Reserve(&v->cargo, cap, st->xy, next_station);
			}
...
static void LoadUnloadVehicle(Vehicle *front)
{
	...
	for (Vehicle *v = front; v != NULL; v = v->Next()) {
		...
		if (HasBit(v->vehicle_flags, VF_CARGO_UNLOADING) && (front->current_order.GetUnloadType() & OUFB_NO_UNLOAD) == 0) {
			...
			if (!HasBit(ge->acceptance_pickup, GoodsEntry::GES_ACCEPTANCE) && v->cargo.ActionCount(VehicleCargoList::A_DELIVER) > 0) {
				/* The station does not accept our goods anymore. */
				if (front->current_order.GetUnloadType() & (OUFB_TRANSFER | OUFB_UNLOAD)) {
					/* Transfer instead of delivering. */
					v->cargo.Reassign(v->cargo.ActionCount(VehicleCargoList::A_DELIVER), VehicleCargoList::A_DELIVER, VehicleCargoList::A_TRANSFER);
				} else {
					/* Keep instead of delivering. This may lead to no cargo being unloaded, so ...*/
					v->cargo.Reassign(v->cargo.ActionCount(VehicleCargoList::A_DELIVER), VehicleCargoList::A_DELIVER, VehicleCargoList::A_KEEP);
		...
		if (front->current_order.IsRefit() && artic_part == 1 && IsArticulatedVehicleEmpty(v) &&
			...
			w = v_start;
			do {
				st->goods[w->cargo_type].cargo.Reserve(&w->cargo, w->cargo_cap, st->xy, next_station);
		...
		if (cap_left > 0 && (v->cargo.ActionCount(VehicleCargoList::A_LOAD) > 0 || !ge->cargo.Empty())) {
			...
			int loaded = ge->cargo.Load(&v->cargo, cap_left, st->xy, next_station);
