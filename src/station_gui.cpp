...
/**
 * Predicate to sort CargoDataEntries with stl algorithms. Can sort by
 * different CargoSortTypes.
 */
class CargoSorter {
public:
 ...
 * Anonymous structs in anonymous unions are supported in most compilers, but
 * not part of the standard. The problem here can not be cleanly solved with
 * templates and inheritance as that would require separate types for each
 * "column" of the station GUI (there is no way to recursively typedef template
 * classes). With the general purpose CargoDataEntry I'm avoiding that and
 * reducing code size significantly.
 */
class CargoAndTransfer {
	CargoID cargo;
	bool transfer;
public:
	operator CargoID() const { return this->cargo; }

	/**
	 * When setting the CargoID, initialize transfer. We can't define a
	 * constructor as that could not be used in a union, either.
	 * @param cargo New CargoID.
	 * @return The just set cargo.
	 */
	...
	bool HasTransfer() const { return this->transfer; }
	void SetTransfer(bool transfer) { this->transfer = transfer; }
};
...
class CargoDataEntry {
public:
	CargoDataEntry() : parent(NULL), station(INVALID_STATION),
			num_children(0), count(0),
	...
	/**
	 * Return if there is transferred cargo for this entry. This only returns
	 * something useful if this is a "first column" entry.
	 * @return If there is transferred cargo.
	 */
	bool HasTransfer() const { return this->cargo.HasTransfer(); }

	/**
	 * Set the "transfer" member of the cargo struct. This tells if there is
	 * transferred cargo at this station. This only makes sense if it's a
	 * "first column" entry.
	 * @param transfer If there is transferred cargo.
	 */
	void SetTransfer(bool transfer) { this->cargo.SetTransfer(transfer); }

	void Clear();
private:

	CargoDataEntry(StationID station, uint count, CargoDataEntry *parent) :
			parent(parent), station(station), num_children(0), count(count),
			children(new CargoDataSet) {}

	CargoDataEntry(CargoID cargo, uint count, CargoDataEntry *parent) :
			parent(parent), num_children(0), count(count), children(new CargoDataSet)
	{
		this->cargo = cargo;
	}

	CargoDataEntry(StationID station) : parent(NULL), station(station),
			num_children(0), count(0), children(NULL) {}

	CargoDataEntry(CargoID cargo) : parent(NULL), num_children(0),
	...
	const union {
		...
		CargoAndTransfer cargo; ///< ID of the cargo this entry is associated with.
	};
...
/**
 * Recursively increment the number of children, up to the root of the tree.
 */
...
/**
 * Sort the children of this set in a different way.
 * @param type New sorting predicate to be applied.
...
/**
 * Retrieve the CargoDataEntry pointed to by an iterator, or NULL if the
 * iterator points to end().
 * @param i Iterator into the set of children.
 * @return Child pointed to by the iterator or NULL.
 */
...
/**
 * Sorting operator.
 * @param cd1 First entry to be compared.
 * @param cd2 Second entry to be compared.
 * @return If cd1 < cd2 according to the current sort criteria.
 */
 ...
 * @tparam Tid ID class; typically StationID or CargoID.
 * @param id1 First ID to compare.
 * @param id2 Second ID to compare.
 * @return If cd1 < cd2 in ascending, or id2 < id1 in descending mode.
 */
template<class Tid>
bool CargoSorter::SortId(Tid id1, Tid id2) const
{
	return (this->order == SO_ASCENDING) ? id1 < id2 : id2 < id1;
}

/**
 * Helper for sorting by cargo count.
 * @param cd1 First CargoDataEntry to compare.
 * @param cd2 Second CargoDataEntry to compare.
 * @return If cd1 is to be inserted above cd2 given current sort criteria.
 */
 ...
 * @param st1 ID of first station to compare.
 * @param st2 ID of second station to compare.
 * @return If st1 is to be inserted above st2 given current sort criteria.
 */
...
struct StationViewWindow : public Window {
	...
	/**
	 * Show a certain cargo entry characterized by source/next/dest station,
	 * cargo ID and amount of cargo at the right place in the cargo view. I.e.
	 * update as many rows as are expanded following that characterization.
		...
		for (int i = 0; i < NUM_COLUMNS && expand != NULL; ++i) {
			switch (groupings[i]) {
				case GR_CARGO:
					...
					data->SetTransfer(source != this->window_number);
	...
	/**
	 * Rebuild the cache for estimated destinations which is used to quickly
	 * show the "destination" entries even if we actually don't know the
	 * destination of a certain packet from just looking at it.
	 * @param cargo Cargo to recalculate the cache for.
	 */
	void RecalcDestinations(CargoID cargo)
	{
		const Station *st = Station::Get(this->window_number);
		CargoDataEntry *cargo_entry = cached_destinations.InsertOrRetrieve(cargo);
		cargo_entry->Clear();

		const FlowStatMap &flows = st->goods[cargo].flows;
	 ...
	 * source station and next hop and save the result as children of the given
	 * CargoDataEntry.
	 ...
	 * @param data CargoDataEntry to save the results in.
	 */
	void EstimateDestinations(CargoID cargo, StationID source, StationID next, uint count, CargoDataEntry *data)
	{
		if (Station::IsValidID(next) && Station::IsValidID(source)) {
			...
			if (tmp.GetCount() == 0) {
				data->InsertOrRetrieve(INVALID_STATION)->Update(count);
			} else {
					...
					for (CargoDataSet::iterator i = tmp.Begin(); i != tmp.End() && sum_estimated < count; ++i) {
						...
						if (estimate > 0) {
							if (child->GetStation() == next) {
								data->InsertOrRetrieve(next)->Update(estimate);
							} else {
								EstimateDestinations(cargo, source, child->GetStation(), estimate, data);
							}
						}
					}
	 ...
	 * @param cargo ID of cargo to show.
	 * @param flows Current station's flows for that cargo.
	 * @param data CargoDataEntry to save the results in.
	 */
	void BuildFlowList(CargoID cargo, const FlowStatMap &flows, CargoDataEntry *data)
	{
		...
		for (FlowStatMap::const_iterator it = flows.begin(); it != flows.end(); ++it) {
			...
			for (FlowStat::SharesMap::const_iterator flow_it = shares->begin(); flow_it != shares->end(); ++flow_it) {
				...
				for (CargoDataSet::iterator dest_it = via_entry->Begin(); dest_it != via_entry->End(); ++dest_it) {
					...
					ShowCargo(data, cargo, from, flow_it->second, dest_entry->GetStation(), dest_entry->GetCount());
				}
			}
		}
	}
	 ...
	 * @param cargo ID of cargo to show.
	 * @param packets Current station's cargo list for that cargo.
	 * @param data CargoDataEntry to save the result in.
	 */
	void BuildCargoList(CargoID cargo, const StationCargoList &packets, CargoDataEntry *data)
	{
		const CargoDataEntry *source_dest = this->cached_destinations.Retrieve(cargo);
		for (StationCargoList::ConstIterator it = packets.Packets()->begin(); it != packets.Packets()->end(); it++) {
			...
			if (source_entry == NULL) {
				this->ShowCargo(data, cargo, cp->SourceStation(), next, INVALID_STATION, cp->Count());
				continue;
			}
			...
			for (CargoDataSet::iterator dest_it = via_entry->Begin(); dest_it != via_entry->End(); ++dest_it) {
				...
				this->ShowCargo(data, cargo, cp->SourceStation(), next, dest_entry->GetStation(), val);
			}
		}
		this->ShowCargo(data, cargo, NEW_STATION, NEW_STATION, NEW_STATION, packets.ReservedCount());
	}

	/**
	 * Build up the cargo view for all cargoes.
	 * @param data Root cargo entry to save all results in.
	 * @param st Station to calculate the cargo view from.
	 */
	void BuildCargoList(CargoDataEntry *data, const Station *st)
	{
		for (CargoID i = 0; i < NUM_CARGO; i++) {
			...
			if (this->current_mode == MODE_WAITING) {
				this->BuildCargoList(i, st->goods[i].cargo, data);
			} else if (_settings_game.linkgraph.GetDistributionType(i) != DT_MANUAL){
				this->BuildFlowList(i, st->goods[i].flows, data);
			}
		}
	}
	 ...
	 * @param data Row to be marked as expanded.
	 */
	 ...
	 * @param data Root entry for all cargo to be drawn.
	 ...
	 */
	int DrawEntries(CargoDataEntry *data, Rect &r, int pos, int maxrows, int column, CargoID cargo = CT_INVALID)
	{
		if (this->sortings[column] == ST_AS_GROUPING) {
			if (this->groupings[column] != GR_CARGO) {
				data->Resort(ST_STATION_STRING, this->sort_orders[column]);
			}
		} else {
			data->Resort(ST_COUNT, this->sort_orders[column]);
		}
		for (CargoDataSet::iterator i = data->Begin(); i != data->End(); ++i) {
			CargoDataEntry *cd = *i;
			...
			if (pos > -maxrows && pos <= 0) {
				...
				if (column < NUM_COLUMNS - 1) {
					...
					if (cd->GetNumChildren() > 0) {
						...
						if (grouping == GR_CARGO && (list.ReservedCount() > 0 || cd->HasTransfer())) {
							sym = "+";
						}
					}
	 ...
	 * @tparam Tid Type of ID of entry to be expanded or collapsed.
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		switch (widget) {
			...
			case WID_SV_SORT_BY: {
				int sorting_index = this->current_mode * 2 + (this->sortings[1] == ST_AS_GROUPING ? 0 : 1);
				ShowDropDownMenu(this, _sort_names, sorting_index, WID_SV_SORT_BY, 0, 0);
				break;
			}
...
/**
 * String names for the available sort orders and modes.
 */
...
/**
 * String names for the available group orders.
 */
