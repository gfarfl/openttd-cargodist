 ...
 * @param next ID of next the station the cargo wants to go next.
 ...
 */
void VehicleCargoList::BalanceReserved(VehicleCargoList *other, uint max_move)
{
	uint move = min((this->reserved_count - other->reserved_count) / 2, max_move);
	Iterator it(this->reserved.begin());
	while (it != this->reserved.end() && move > 0) {
		CargoPacket *cp = *it;
		if (cp->count <= move) {
			this->RemoveFromCache(cp);
			this->reserved_count -= cp->count;
